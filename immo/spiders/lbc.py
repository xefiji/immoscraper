...
class LbcSpider(scrapy.Spider):
	...
	def send_mail(self):
		cps = [38120,38950,38700,38330,38920,38660,38570,38190,38420,38410,38320,38610,38560,3845,38760,38640,38180,38170,38600,38360,38250,38400,38130,38240,38560,38000,38800,38220,38340]
		Session = sessionmaker(bind=self.engine)
		session = Session()
		...
		for maison in res:
			...
			maison_row += "<td>" + "<strong>{0} {1}</strong>".format(unidecode(maison_data['city']) if maison_data['city'] is not None else "-", maison_data['cp'] if maison_data['cp'] is not None else "-") + "</td>"
			maison_row += "<td>" + "<strong>" + str(maison_data['price']) + "</strong>" if maison_data['price'] is not None else "-" + "</td>"
			maison_row += "<td>" + str(maison_data['published_at']) if maison_data['published_at'] is not None else "-" + "</td>"
			maison_row += "</tr>"
		...
		mailer.send(to=["anais.rossettom@gmail.com"], subject="Nouvelles maisons en ligne :)", body=mail_body, cc=["fxechappe@gmail.com"], mimetype='text/html')
