...
class LbcColocSpider(scrapy.Spider):
    ...
    def send_mail(self):
        ...
        Session = sessionmaker(bind=self.engine)
        session = Session()
        res = session.query(Product).filter(Product.category == self.cat).filter(Product.alert_sent == "0").order_by(
            Product.published_at.desc()).all()
        ...
        for coloc in res:
            ...
            coloc_row = "<tr>"
            img = "<img width='250' alt='image' src='http:" + coloc_data['img'] + "'/>" if coloc_data[
                                                                                               'img'] is not None else "-"
            coloc_row += "<td>" + img + "</td>"
            name = "<a href='http:" + (
            coloc_data['href'] if coloc_data['href'] is not None else "#") + "'>" + unidecode(
                coloc_data['name']) + "</a>" if coloc_data['name'] is not None else "-"
            coloc_row += "<td>" + name + "</td>"
            coloc_row += "<td>" + "<strong>{0} {1}</strong>".format(
                unidecode(coloc_data['city']) if coloc_data['city'] is not None else "-",
                coloc_data['cp'] if coloc_data['cp'] is not None else "-") + "</td>"
            coloc_row += "<td>" + "<strong>" + str(coloc_data['price']) + "</strong>" if coloc_data[
                                                                                             'price'] is not None else "-" + "</td>"
            coloc_row += "<td>" + str(coloc_data['published_at']) if coloc_data[
